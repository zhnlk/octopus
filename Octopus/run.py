...
def main():
    ...
    initialize()
    app.logger.info("Octopus startd on %s:%s username:%s/password:%s with %s servers:%s" % (
...
def parse_opts(config):
    ...
    parser.add_option("--database-url",
                      help='Octopus metadata database default: %s' % config.get('SQLALCHEMY_DATABASE_URI'),
                      dest='database_url',
